# core/dictionary.py

# Dicionário oficial de moedas do sistema
# Chave: Sigla (Symbol) -> Valor: Nome (Name)
MOEDAS_DICT = {
    "USDC": "USD Coin",
    "ETH": "Ethereum",
    "BTC": "Bitcoin",
    "BNB": "BNB",
    "LTC": "Litecoin",
    "NEO": "NEO",
    "QTUM": "Qtum",
    "EOS": "EOS",
    "SNT": "Status",
    "BNT": "Bancor",
    "BCC": "Bitcoin Connect",
    "GAS": "Gas",
    "USDT": "TetherUS",
    "HSR": "Hshare",
    "OAX": "OAX",
    "DNT": "district0x",
    "MCO": "MCO",
    "ICN": "Iconomi",
    "WTC": "Waltonchain",
    "LRC": "Loopring",
    "YOYO": "YOYOW",
    "OMG": "OMG Network",
    "ZRX": "0x",
    "STRAT": "Stratis",
    "SNGLS": "SingularityDTV",
    "BQX": "BQX",
    "KNC": "Kyber Network",
    "FUN": "FUNToken",
    "SNM": "SONM",
    "IOTA": "MIOTA",
    "LINK": "Chainlink",
    "XVG": "Verge",
    "SALT": "SALT",
    "MDA": "Moeda Loyalty Points",
    "MTL": "Metal",
    "SUB": "Substratum",
    "ETC": "Ethereum Classic",
    "MTH": "Monetha",
    "ENG": "Enigma",
    "ZEC": "Zcash",
    "AST": "AirSwap",
    "DASH": "Dash",
    "BTG": "Bitcoin Gold",
    "EVX": "Everex",
    "REQ": "Request",
    "VIB": "Viberate",
    "TRX": "TRON",
    "POWR": "PowerLedger",
    "ARK": "Ark",
    "XRP": "XRP",
    "MOD": "Modum",
    "ENJ": "Enjin Coin",
    "STORJ": "Storj",
    "VEN": "VeChain",
    "KMD": "Komodo",
    "NULS": "NULS",
    "RCN": "Ripio Credit Network",
    "RDN": "Raiden Network Token",
    "XMR": "Monero",
    "DLT": "Agrello",
    "AMB": "Ambrosus",
    "BAT": "Basic Attention Token",
    "BCPT": "Blockmason Credit Protocol",
    "ARN": "Aeron",
    "GVT": "Genesis Vision",
    "CDT": "Blox",
    "GXS": "GXChain",
    "POE": "Po.et",
    "QSP": "Quantstamp",
    "BTS": "BitShares",
    "XZC": "Zcoin",
    "LSK": "Lisk",
    "TNT": "Tierion",
    "FUEL": "Etherparty",
    "MANA": "Decentraland",
    "BCD": "Bitcoin Diamond",
    "DGD": "DigixDAO",
    "ADX": "AdEx",
    "ADA": "Cardano",
    "PPT": "Populous",
    "CMT": "CyberMiles",
    "XLM": "Stellar",
    "CND": "Cindicator",
    "LEND": "ETHLend",
    "WABI": "Tael",
    "TNB": "Time New Bank",
    "WAVES": "Waves",
    "GTO": "Gifto",
    "ICX": "ICON",
    "OST": "OST",
    "ELF": "aelf",
    "AION": "Aion",
    "NEBL": "Neblio",
    "BRD": "Bread",
    "EDO": "Eidoo",
    "WINGS": "Wings",
    "NAV": "NAV Coin",
    "LUN": "Lunyr",
    "TRIG": "Triggers",
    "APPC": "AppCoins",
    "VIBE": "VIBE",
    "RLC": "iExec RLC",
    "INS": "Insolar",
    "PIVX": "PIVX",
    "IOST": "IOST",
    "CHAT": "ChatCoin",
    "STEEM": "Steem",
    "NANO": "Nano",
    "VIA": "Viacoin",
    "BLZ": "Bluzelle",
    "AE": "Aeternity",
    "RPX": "Red Pulse",
    "NCASH": "Nucleus Vision",
    "POA": "POA Network",
    "ZIL": "Zilliqa",
    "ONT": "Ontology",
    "STORM": "Storm",
    "XEM": "NEM",
    "WAN": "Wanchain",
    "WPR": "WePower",
    "QLC": "QLC Chain",
    "SYS": "Syscoin",
    "GRS": "Groestlcoin",
    "CLOAK": "CloakCoin",
    "GNT": "Golem",
    "LOOM": "Loom Network",
    "BCN": "Bytecoin",
    "REP": "Augur",
    "TUSD": "TrueUSD",
    "ZEN": "Horizen",
    "SKY": "Skycoin",
    "CVC": "Civic",
    "THETA": "Theta Token",
    "IOTX": "IoTeX",
    "QKC": "QuarkChain",
    "AGI": "SingularityNET",
    "NXS": "Nexus",
    "DATA": "Streamr",
    "SC": "Siacoin",
    "NPXS": "Pundi X",
    "KEY": "Selfkey",
    "NAS": "Nebulas",
    "MFT": "Hifi Finance",
    "DENT": "Dent",
    "ARDR": "Ardor",
    "HOT": "Holo",
    "VET": "VeChain",
    "DOCK": "Dock",
    "POLY": "Polymath",
    "PHX": "Red Pulse Phoenix",
    "HC": "HyperCash",
    "GO": "GoChain",
    "PAX": "PAX Gold",
    "RVN": "Ravencoin",
    "DCR": "Decred",
    "MITH": "Mithril",
    "BCHABC": "Bitcoin Cash ABC",
    "BCHSV": "Bitcoin Cash SV",
    "REN": "Ren",
    "BTT": "BitTorrent",
    "USDS": "Stably USD",
    "ONG": "Ontology Gas",
    "FET": "Fetch.ai",
    "CELR": "Celer Network",
    "MATIC": "Polygon",
    "ATOM": "Cosmos",
    "PHB": "Phoenix Global",
    "TFUEL": "Theta Fuel",
    "ONE": "Harmony",
    "FTM": "Fantom",
    "BTCB": "Bitcoin BEP2",
    "ALGO": "Algorand",
    "USDSB": "USDSB",
    "ERD": "Elrond",
    "DOGE": "Dogecoin",
    "DUSK": "Dusk",
    "BGBP": "Binance GBP Stable Coin",
    "ANKR": "Ankr",
    "WIN": "WINkLink",
    "COS": "Contentos",
    "COCOS": "COCOS BCX",
    "TOMO": "TomoChain",
    "PERL": "Perlin",
    "CHZ": "Chiliz",
    "BAND": "Band Protocol",
    "BUSD": "BUSD",
    "BEAM": "Beam",
    "XTZ": "Tezos",
    "HBAR": "Hedera",
    "NKN": "NKN",
    "STX": "Stacks",
    "KAVA": "Kava",
    "NGN": "Nigerian Naira",
    "ARPA": "ARPA Chain",
    "CTXC": "Cortex",
    "BCH": "Bitcoin Cash",
    "RUB": "Russian Ruble",
    "TROY": "TROY",
    "VITE": "VITE",
    "FTT": "FTT",
    "TRY": "Turkish Lira",
    "EUR": "Euro",
    "OGN": "Origin Protocol",
    "DREP": "Drep",
    "BULL": "3X Long Bitcoin Token",
    "BEAR": "3X Short Bitcoin Token",
    "ETHBULL": "3X Long Ethereum Token",
    "ETHBEAR": "3X Short Ethereum Token",
    "TCT": "TokenClub",
    "WRX": "WazirX",
    "LTO": "LTO Network",
    "EOSBULL": "3X Long EOS Token",
    "EOSBEAR": "3X Short EOS Token",
    "XRPBULL": "3X Long XRP Token",
    "XRPBEAR": "3X Short XRP Token",
    "MBL": "MovieBloc",
    "COTI": "COTI",
    "BNBBULL": "3X Long BNB Token",
    "BNBBEAR": "3X Short BNB Token",
    "STPT": "Standard Tokenization Protocol",
    "ZAR": "South African Rand",
    "BKRW": "Binance KRW",
    "SOL": "Solana",
    "IDRT": "Rupiah Token",
    "CTSI": "Cartesi",
    "HIVE": "Hive",
    "CHR": "Chromia",
    "BTCUP": "BTCUP",
    "BTCDOWN": "BTCDOWN",
    "MDT": "Measurable Data Token",
    "STMX": "StormX",
    "IQ": "IQ",
    "PNT": "pNetwork",
    "GBP": "Pound Sterling",
    "DGB": "DigiByte",
    "UAH": "Ukrainian Hryvnia",
    "COMP": "Compound",
    "BIDR": "BIDR",
    "SXP": "SXP",
    "SNX": "Synthetix",
    "ETHUP": "ETHUP",
    "ETHDOWN": "ETHDOWN",
    "ADAUP": "ADAUP",
    "ADADOWN": "ADADOWN",
    "LINKUP": "LINKUP",
    "LINKDOWN": "LINKDOWN",
    "VTHO": "VeThor Token",
    "IRIS": "IRISnet",
    "MKR": "Maker",
    "DAI": "Dai",
    "RUNE": "THORChain",
    "AUD": "Australian Dollar",
    "FIO": "FIO Protocol",
    "BNBUP": "BNBUP",
    "BNBDOWN": "BNBDOWN",
    "XTZUP": "XTZUP",
    "XTZDOWN": "XTZDOWN",
    "AVA": "Travala.com",
    "BAL": "Balancer",
    "YFI": "yearn.finance",
    "JST": "JUST",
    "SRM": "Serum",
    "ANT": "Aragon",
    "CRV": "Curve DAO Token",
    "SAND": "The Sandbox",
    "OCEAN": "Ocean Protocol",
    "NMR": "Numeraire",
    "DOT": "Polkadot",
    "LUNA": "Terra",
    "IDEX": "IDEX",
    "RSR": "Reserve Rights",
    "WNXM": "Wrapped NXM",
    "TRB": "Tellor",
    "BZRX": "bZx Protocol",
    "WBTC": "Wrapped Bitcoin",
    "SUSHI": "SushiSwap",
    "YFII": "DFI.Money",
    "KSM": "Kusama",
    "EGLD": "MultiversX",
    "DIA": "DIA",
    "UMA": "UMA",
    "EOSUP": "EOSUP",
    "EOSDOWN": "EOSDOWN",
    "TRXUP": "TRXUP",
    "TRXDOWN": "TRXDOWN",
    "XRPUP": "XRPUP",
    "XRPDOWN": "XRPDOWN",
    "DOTUP": "DOTUP",
    "DOTDOWN": "DOTDOWN",
    "BEL": "Bella Protocol",
    "WING": "Wing",
    "SWRV": "Swerve",
    "LTCUP": "LTCUP",
    "LTCDOWN": "LTCDOWN",
    "CREAM": "Cream Finance",
    "UNI": "Uniswap",
    "NBS": "New BitShares",
    "OXT": "Orchid",
    "SUN": "SUN",
    "AVAX": "Avalanche",
    "HNT": "Helium",
    "BAKE": "BakeryToken",
    "BURGER": "Burger",
    "FLM": "Flamingo",
    "SCRT": "Secret",
    "CAKE": "PancakeSwap",
    "SPARTA": "Spartan Protocol",
    "UNIUP": "UNIUP",
    "UNIDOWN": "UNIDOWN",
    "ORN": "Orion Protocol",
    "UTK": "Utrust",
    "XVS": "Venus",
    "ALPHA": "Stella",
    "VIDT": "VIDT Datalink",
    "AAVE": "Aave",
    "NEAR": "NEAR Protocol",
    "SXPUP": "SXPUP",
    "SXPDOWN": "SXPDOWN",
    "FIL": "Filecoin",
    "FILUP": "FILUP",
    "FILDOWN": "FILDOWN",
    "YFIUP": "YFIUP",
    "YFIDOWN": "YFIDOWN",
    "INJ": "Injective",
    "AERGO": "Aergo",
    "EASY": "EasyFi",
    "AUDIO": "Audius",
    "CTK": "Shentu",
    "BCHUP": "BCHUP",
    "BCHDOWN": "BCHDOWN",
    "BOT": "Botritto",
    "BRL": "Brazilian Real",
    "AKRO": "Akropolis",
    "KP3R": "Keep3rV1",
    "AXS": "Axie Infinity",
    "HARD": "Kava Lend",
    "RENBTC": "renBTC",
    "SLP": "Smooth Love Potion",
    "CVP": "PowerPool",
    "STRAX": "Stratis",
    "FOR": "ForTube",
    "UNFI": "Unifi Protocol DAO",
    "FRONT": "Frontier",
    "BCHA": "Bitcoin Cash ABC",
    "ROSE": "Oasis Network",
    "HEGIC": "Hegic",
    "AAVEUP": "AAVEUP",
    "AAVEDOWN": "AAVEDOWN",
    "PROM": "Prom",
    "SKL": "SKALE",
    "SUSD": "sUSD",
    "COVER": "Cover Protocol",
    "GLM": "Golem",
    "GHST": "Aavegotchi",
    "SUSHIUP": "SUSHIUP",
    "SUSHIDOWN": "SUSHIDOWN",
    "XLMUP": "XLMUP",
    "XLMDOWN": "XLMDOWN",
    "DF": "dForce",
    "GRT": "The Graph",
    "JUV": "Juventus Fan Token",
    "PSG": "Paris Saint-Germain Fan Token",
    "BVND": "Binance VND Stable Coin",
    "1INCH": "1inch",
    "REEF": "Reef",
    "OG": "OG Fan Token",
    "ATM": "Atletico De Madrid Fan Token",
    "ASR": "AS Roma Fan Token",
    "CELO": "Celo",
    "RIF": "RSK Infrastructure Framework",
    "BTCST": "BTC Standard Hashrate Token",
    "TRU": "TrueFi",
    "DEXE": "DeXe",
    "CKB": "Nervos Network",
    "TWT": "Trust Wallet Token",
    "FIRO": "Firo",
    "BETH": "Beacon ETH",
    "PROS": "Prosper",
    "LIT": "Litentry",
    "VAI": "VAI",
    "SFP": "SafePal",
    "FXS": "Frax Share",
    "DODO": "DODO",
    "UFT": "UniLend",
    "ACM": "AC Milan Fan Token",
    "AUCTION": "Bounce Token",
    "PHA": "Phala Network",
    "TVK": "Virtua",
    "BADGER": "Badger DAO",
    "FIS": "Stafi",
    "OM": "MANTRA",
    "POND": "Marlin",
    "DEGO": "Dego Finance",
    "ALICE": "My Neighbor Alice",
    "BIFI": "Beefy.Finance",
    "LINA": "Linear",
    "PERP": "Perpetual Protocol",
    "RAMP": "RAMP",
    "SUPER": "SuperFarm",
    "CFX": "Conflux",
    "EPS": "Ellipsis",
    "AUTO": "Auto",
    "TKO": "Tokocrypto",
    "PUNDIX": "Pundi X (New)",
    "TLM": "Alien Worlds",
    "1INCHUP": "1INCHUP",
    "1INCHDOWN": "1INCHDOWN",
    "MIR": "Mirror Protocol",
    "BAR": "FC Barcelona Fan Token",
    "FORTH": "Ampleforth Governance Token",
    "EZ": "Easy",
    "SHIB": "SHIBA INU",
    "ICP": "Internet Computer",
    "AR": "Arweave",
    "POLS": "Polkastarter",
    "MDX": "Mdex",
    "MASK": "Mask Network",
    "LPT": "Livepeer",
    "AGIX": "SingularityNET",
    "NU": "NuCypher",
    "ATA": "Automata Network",
    "GTC": "Gitcoin",
    "TORN": "Tornado Cash",
    "KEEP": "Keep Network",
    "ERN": "Ethernity",
    "KLAY": "Klaytn",
    "BOND": "BarnBridge",
    "MLN": "Enzyme",
    "QUICK": "QuickSwap",
    "C98": "Coin98",
    "CLV": "Clover Finance",
    "QNT": "Quant",
    "FLOW": "Flow",
    "XEC": "eCash",
    "MINA": "Mina",
    "RAY": "Raydium",
    "FARM": "Harvest Finance",
    "ALPACA": "Alpaca Finance",
    "MBOX": "MOBOX",
    "VGX": "Voyager Token",
    "WAXP": "WAX",
    "TRIBE": "Tribe",
    "GNO": "Gnosis",
    "DYDX": "dYdX",
    "USDP": "Pax Dollar",
    "GALA": "Gala",
    "ILV": "Illuvium",
    "YGG": "Yield Guild Games",
    "FIDA": "Bonfida",
    "AGLD": "Adventure Gold",
    "RAD": "Radicle",
    "BETA": "Beta Finance",
    "RARE": "SuperRare",
    "SSV": "SSV Network",
    "LAZIO": "Lazio Fan Token",
    "CHESS": "Tranchess",
    "DAR": "Mines of Dalarnia",
    "BNX": "BinaryX",
    "RGT": "Rari Governance Token",
    "MOVR": "Moonriver",
    "CITY": "Manchester City Fan Token",
    "ENS": "Ethereum Name Service",
    "QI": "BENQI",
    "PORTO": "FC Porto Fan Token",
    "JASMY": "JasmyCoin",
    "AMP": "Amp",
    "PLA": "PlayDapp",
    "PYR": "Vulcan Forged PYR",
    "RNDR": "Render Token",
    "ALCX": "Alchemix",
    "SANTOS": "Santos FC Fan Token",
    "MC": "Merit Circle",
    "ANY": "Anyswap",
    "BICO": "Biconomy",
    "FLUX": "Flux",
    "VOXEL": "Voxies",
    "HIGH": "Highstreet",
    "CVX": "Convex Finance",
    "PEOPLE": "ConstitutionDAO",
    "OOKI": "Ooki Protocol",
    "SPELL": "Spell Token",
    "UST": "UST",
    "JOE": "JOE",
    "ACH": "Alchemy Pay",
    "IMX": "Immutable X",
    "GLMR": "Moonbeam",
    "LOKA": "League of Kingdoms Arena",
    "API3": "API3",
    "BTTC": "BitTorrent",
    "ACA": "Acala",
    "ANC": "Anchor Protocol",
    "BDOT": "BDOT",
    "XNO": "XNO",
    "WOO": "WOO Network",
    "ALPINE": "Alpine F1 Team Fan Token",
    "T": "Threshold",
    "ASTR": "Astar",
    "GMT": "STEPN",
    "KDA": "Kadena",
    "APE": "ApeCoin",
    "BSW": "Biswap",
    "MULTI": "Multichain",
    "MOB": "MobileCoin",
    "NEXO": "Nexo",
    "REI": "REI Network",
    "GAL": "Galxe",
    "LDO": "Lido DAO",
    "EPX": "Ellipsis",
    "LUNC": "Terra Classic",
    "USTC": "TerraClassicUSD",
    "OP": "Optimism",
    "LEVER": "LeverFi",
    "STG": "Stargate Finance",
    "GMX": "GMX",
    "POLYX": "Polymesh",
    "APT": "Aptos",
    "PLN": "Polish Zloty",
    "OSMO": "Osmosis",
    "HFT": "Hashflow",
    "HOOK": "Hooked Protocol",
    "MAGIC": "Magic",
    "RON": "Ronin",
    "HIFI": "Hifi Finance",
    "RPL": "Rocket Pool",
    "GFT": "Gifto",
    "GNS": "Gains Network",
    "SYN": "Synapse",
    "LQTY": "Liquity",
    "ID": "SPACE ID",
    "ARB": "Arbitrum",
    "RDNT": "Radiant Capital",
    "MXN": "Mexican Peso",
    "CZK": "Czech Koruna",
    "TAO": "Bittensor",
    "OMNI": "Omni Network",
    "REZ": "Renzo",
    "BB": "BounceBit",
    "NOT": "Notcoin",
    "IO": "io.net",
    "ZK": "ZKsync",
    "LISTA": "Lista DAO",
    "ZRO": "LayerZero",
    "BANANA": "Banana Gun",
    "RENDER": "Render",
    "TON": "Toncoin",
    "DOGS": "Dogs",
    "EURI": "EURI",
    "SLF": "Self Chain",
    "POL": "Polygon Ecosystem Token",
    "NEIRO": "Neiro",
    "TURBO": "Turbo",
    "1MBABYDOGE": "1MBABYDOGE",
    "CATI": "Catizen",
    "HMSTR": "Hamster Kombat",
    "EIGEN": "EigenLayer",
    "BNSOL": "Binance Staked SOL",
    "SCR": "Scroll",
    "LUMIA": "Lumia",
    "KAIA": "Kaia",
    "COW": "CoW Protocol",
    "CETUS": "Cetus Protocol",
    "PNUT": "Peanut the Squirrel",
    "ACT": "Act I : The AI Prophecy",
    "USUAL": "Usual",
    "ACX": "Across Protocol",
    "ORCA": "Orca",
    "MOVE": "Movement",
    "ME": "Magic Eden",
    "VELODROME": "Velodrome Finance",
    "VANA": "Vana",
    "1000CAT": "1000CAT",
    "PENGU": "Pudgy Penguins",
    "BIO": "BIO Protocol",
    "D": "D",
    "AIXBT": "AIXBT",
    "CGPT": "ChainGPT",
    "COOKIE": "Cookie",
    "S": "S",
    "SOLV": "Solv Protocol",
    "TRUMP": "OFFICIAL TRUMP",
    "ANIME": "AnimeCoin",
    "BERA": "Bera",
    "1000CHEEMS": "1000CHEEMS",
    "TST": "The Standard Token",
    "LAYER": "LayerAI",
    "HEI": "HEI",
    "KAITO": "Kaito",
    "SHELL": "Shell",
    "RED": "Red Token",
    "GPS": "GPS",
    "EPIC": "Epic Cash",
    "BMT": "BMToken",
    "FORM": "Form Token",
    "XUSD": "XUSD",
    "NIL": "Nil",
    "PARTI": "Partisia Blockchain",
    "MUBARAK": "MUBARAK",
    "TUT": "Tutellus",
    "BROCCOLI714": "BROCCOLI714",
    "BANANAS31": "BANANAS31",
    "GUN": "Gunstar Metaverse",
    "BABY": "BabySwap",
    "ONDO": "Ondo",
    "BIGTIME": "Big Time",
    "VIRTUAL": "Virtual Protocol",
    "KERNEL": "Kernel",
    "WCT": "WalletConnect",
    "HYPER": "Hyper",
    "INIT": "Init Token",
    "SIGN": "Sign Token",
    "STO": "STO",
    "SYRUP": "Syrup",
    "KMNO": "Kamino",
    "SXT": "Space and Time",
    "NXPC": "NXPC",
    "AWE": "Awe",
    "HAEDAL": "Haedal",
    "USD1": "USD1",
    "HUMA": "Huma",
    "A": "A",
    "SOPH": "SophiaVerse",
    "RESOLV": "Resolv",
    "HOME": "Homeverse",
    "SPK": "SPK Network",
    "NEWT": "Newton",
    "SAHARA": "Sahara",
    "LA": "LA",
    "ERA": "Era Token",
    "C": "C",
    "TREE": "Tree Token",
    "A2Z": "A2Z Token",
    "TOWNS": "Town Star",
    "PROVE": "Prove",
    "BFUSD": "BFUSD",
    "PLUME": "Plume",
    "DOLO": "Dolo",
    "MITO": "Mito",
    "WLFI": "World Liberty Financial",
    "SOMI": "Somi",
    "OPEN": "Open Custody Protocol",
    "USDE": "USDE",
    "LINEA": "Linea",
    "HOLO": "Holo",
    "PUMP": "Pump",
    "AVNT": "Avent",
    "ZKC": "ZKChain",
    "BARD": "Bard",
    "0G": "0G",
    "HEMI": "Hemi",
    "XPL": "XPL",
    "MIRA": "Mira",
    "FF": "FF Token",
    "EDEN": "Eden Network",
    "NOM": "Onomy Protocol",
    "2Z": "2Z Token",
    "MORPHO": "Morpho",
    "ASTER": "Aster",
    "WAL": "The Walrus Protocol",
    "EUL": "Euler",
    "ENSO": "Enso",
    "YB": "YB Token",
    "ZBT": "ZBIT",
    "TURTLE": "Turtle",
    "GIGGLE": "Giggle",
    "F": "F",
    "KITE": "Kite",
    "MMT": "MMT",
    "SAPIEN": "Sapien",
    "ALLO": "Allo",
    "BANK": "Bank",
    "MET": "Metronome"
}

def is_moeda(texto):
    """Verifica se uma palavra é uma moeda válida."""
    return texto.upper() in MOEDAS_DICT

def get_nome_moeda(sigla):
    """Retorna o nome da moeda (ex: 'Bitcoin' para 'BTC')."""
    return MOEDAS_DICT.get(sigla.upper(), sigla)