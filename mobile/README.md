...
Este tutorial apresenta um passo a passo completo para quem deseja criar um aplicativo Flutter do zero, abordando desde a criação do projeto até a explicação da estrutura do código. O exemplo final será um Dashboard simples com menu lateral (Drawer), tabela de dados e gráfico de barras usando o pacote fl_chart.
...
pubspec.yaml Arquivo de configuração do projeto (nome, versão, dependências, assets).

android/ e ios/ Contêm arquivos nativos específicos de cada plataforma.

test/ Armazena testes automatizados do aplicativo.
...
Um aplicativo Flutter é composto por widgets, que são elementos visuais ou funcionais. Tudo em Flutter é um widget: textos, botões, colunas, tabelas e até o próprio app.
